pipeline {
    ...
    environment {
        ...
        scannerHome = tool 'jenkins-tool2' // Pastikan ini sesuai nama tool di Jenkins
    }

    stages {
        stage('SCM Checkout') {
            steps {
                git branch: 'vuln', url: 'https://github.com/Reqyan/devsecops-forum-app.git'
            }
        }
        ...
        stage('Run Migration') {
            steps {
                ...
                sh "docker exec forum_app2_laravel.test_1 php artisan migrate:fresh --seed"
            }
        }

        stage('Set Permissions') {
            steps {
                sh "docker exec forum_app2_laravel.test_1 chmod -R 777 /var/www/html/storage"
            }
        }
        stage('DAST OWASP ZAP') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh 'zap-baseline.py -t http://localhost:82 -r zapbaseline.html -x zapbaseline.xml'
                }
